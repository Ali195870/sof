...
from emotes import Dance_Floor
owners = ['alionardo_','imgodsfavourite']
moderators = ['alionardo_','imgodsfavourite']
class BotDefinition:
...
class Bot(BaseBot):
      ...
      try:
         ...
         if message.lstrip().startswith(("-give","-remove","-here","-tele")):
            ...
            try:
                ...
                elif message.lower().startswith(('-tele')) and  message.lower().endswith("yellow"):   
                  if user.username.lower() in self.moderators:
         ...
         if message.lower().startswith(('-floor2','-2','-dj')):
            if user.username.lower() in self.moderators:
              parts = message.split()
              if len(parts) == 1:
                 await self.highrise.teleport(f"{user.id}", Position(17.5,9.25,2.5))
    ...
    async def on_whisper(self, user: User, message: str ) -> None:
        ...
        if message.startswith("-say"):
            ...
            await self.highrise.walk_to(Position(17, 0,3.5, facing='FrontRight'))
